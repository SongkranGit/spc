...
class Multi_upload  {
	 ...
	 */	
	function go_upload($field = 'userfile' , $prefixImage) {
        $CI =& get_instance(); 
...
//            echo "$fname\n\n<br><br>\n\n";
         ...
         //   $CI->upload->file_name = $CI->upload->_prep_filename($_FILES[$field]['name'][$i]);
			...
			$CI->upload->file_name =$prefixImage."_".time()."_".$count.$CI->upload->get_extension($_FILES[$field]['name'][$i]); // กำหนดชื่อรูปภาพ
